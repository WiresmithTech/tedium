...
fn create_file(elements: usize, channels: usize) -> TdmsFile<Cursor<Vec<u8>>> {
    ...
    let channels = (0..channels)
        ...
        .collect::<Vec<_>>();
    writer
        .write_channels(&channels, &buffer[..], DataLayout::Contigious)
        .unwrap();
    drop(writer);
    file
}
...
fn single_channel_segment(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Single Channel Segment - Contigious");
...
fn multi_channel_segment(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Multi Channel Segment - Contigious");
    group.throughput(criterion::Throughput::Bytes(
        elements as u64 * std::mem::size_of::<f64>() as u64,
    ));
